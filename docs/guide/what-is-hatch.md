...
If you're working on more than one project at a time, you've probably lost track of which one runs on port 3000 and which is on 3001. Or hit a conflict because two things want the same port. Or had to explain to a colleague how to set up their `/etc/hosts` and trust a self-signed certificate.
